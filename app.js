...
function normalize(arr) {
    return arr.map(x => {
        if (typeof x === 'string') return x.trim().toLowerCase().replace(/\s+/g, ' ');
        return x;
    }).filter(x => x && x !== '' && x !== '?' && x !== '0');
}
...
function getRadius(dream) {
    switch (selectedSize) {
        case 'importanza': return 5 + dream.importanza_soggettiva * 0.18;
        case 'bizzarria': return 5 + dream.bizzarria * 0.18;
        case 'ricordo': return 5 + dream.ricordo_percentuale * 0.18;
        default: return 9;
    }
}
...
function setupSVG() {
    ...
    // Initial zoom to fit
    svg.call(zoom.transform, d3.zoomIdentity.translate(width / 2, height / 2).scale(1.6));
}
...
function startSimulation() {
    ...
    simulation = d3.forceSimulation(nodes)
        ...
        .force('gravity', d3.forceRadial(0, 0, 0).strength(d => {
            const deg = degreeMap[d.id] || 0;
            return deg <= 1 ? 0.1 : 0.025;
        }))
        .force('collision', d3.forceCollide().radius(d => getRadius(d.dream) + 8))
...
function showLinkTooltip(event, d) {
    ...
    tooltip
        ...
        .html(`SHARED: ${d.shared.map(s => s.toUpperCase()).join(' · ')}<br>WEIGHT: ${d.weight}`);
}
...
function renderNodes() {
    ...
    nodeEnter.append('circle')
        ...
        .attr('fill', d => d.dream['bianco e nero']
...
function updateNodeSizes() {
    nodeG.selectAll('.node-group').each(function (d) {
        ...
        d3.select(this).select('.node-outer').attr('r', r + 3);
        d3.select(this).select('.node-label').attr('dy', r + 12);
    });
}
...
function showPanel(d, connectedIds) {
    ...
    document.getElementById('bar-durata').style.width = `${(dur / maxDuration) * 100}%`;
    document.getElementById('val-durata').textContent = `${dream.durata_percepita_min}min`;
    ...
    renderSection('panel-ambientazione', t('settingLabel'), ambs, 'tag', 'ambientazione');
    ...
    let personeCount = '';
    if (dream.persone) {
        if (dream.persone.quantita > 0) personeCount = `N=${dream.persone.quantita}`;
        normalize(getArr(dream.persone.tipologia)).forEach(tp => personeTipologie.push(tp));
    }
    ...
    if (personeTipologie.length || personeCount) {
        ...
        const tipoTags = personeTipologie.map(i =>
    ...
    renderSection('panel-prospettiva', t('perspectiveLabel'), prosp, 'tag', 'prospettiva');
    ...
    flagEl.innerHTML = flags.map(f =>
        `<span class="tag flag-tag flag-active">${f.label}</span>`).join('');
...
function highlightByValue(filterKey, value) {
    ...
    nodeG.selectAll('.node-group').each(function (d) {
        ...
        d3.select(this).select('.node-label').style('opacity', match ? 1 : null);
    });
...
function highlightByMood(mood) {
    ...
    const matchingIds = new Set();
    nodeG.selectAll('.node-group').each(function (d) {
        const nodeMoods = (Array.isArray(d.dream.mood_principale)
...
function updateLegend() {
    ...
    dreams.forEach(d => getArr(d.mood_principale).forEach(m => {
...
function setupTicker() {
    const items = dreams
        .filter(d => d.titolo)
        .map(d => `[#${String(d.id).padStart(2, '0')}] ${d.titolo.toUpperCase()}`);
...
function setupDataStream() {
    ...
    const moodCount = {};
    const settingCount = {};
    let totalImportanza = 0, totalBizzarria = 0, totalRicordo = 0;
    let dates = [];
    ...
    const topSetting = Object.entries(settingCount).sort((a, b) => b[1] - a[1])[0];
    ...
    const dateRange = dates.length ? `${dates[dates.length - 1]} → ${dates[0]}` : '—';
